...
from src.pdf_to_image import PDFToImageConverter, pdf_to_images, pdf_to_png_bytes
...
def sample_pdf_bytes():
    try:
        ...
        if not path.exists():
            pytest.skip("reportlab не установлен и нет sample.pdf")
...
def sample_pdf_multipage_bytes():
    """
    Генерирует PDF с несколькими страницами.
    """
    try:
        from reportlab.pdfgen import canvas
        from reportlab.lib.pagesizes import letter
        ...
        if not path.exists():
            pytest.skip("reportlab не установлен и нет multipage.pdf")
        return path.read_bytes()
...
class TestPDFToImageConverter:    
    ...
    def test_pdf_bytes_to_images_success(self, converter, sample_pdf_bytes):        
        ...
        assert len(images) == 1, "Должна быть одна страница"
...
def test_with_real_file():    
    from pathlib import Path
    path = Path("data/test.pdf")
    if not path.exists():
        pytest.skip("No file data/test.pdf")
